...
function ContainerEntryForm({
  ...
  const hasViewPermission = (field) => {
    ...
    if (field === "Demurrage") return true;
    const basicFields = ["ContainerNo", "Status", "ContainerType", "ArrivalDate", "EmptyDate", "EmptyAt", "UnloadedAtDock", "InBound", "OutBound", "Material", "BL"];
  ...
  const fields = [
    { label: "Bill Of Landing", name: "BillOfLanding", type: "text", permission: "BL", required: true, options: [] },
    { label: "Container No", name: "containerNo", type: "text", permission: "ContainerNo", required: true },
              ...
              <div className="space-y-1">
                {emptyImages?.map((img, idx) => {
                  ...
                  const fileUrl = img.isExisting
                    ? `${process.env.REACT_APP_NETWORK}/getDocument/${img.id}`
